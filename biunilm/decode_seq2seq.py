...
def main():
    ...
    pair_num_relation = 0
    bi_uni_pipeline = [
        seq2seq_loader.Preprocess4Decoder(list(tokenizer.vocab.keys()), tokenizer.convert_tokens_to_ids,
    ...
    tmp = sorted(bi_uni_pipeline[0].c_indexer.items(), key=lambda p: p[1])
    print(tmp[:10])
    sys.stdout.flush()
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        if '<nan>' not in c_indexer.keys():
            n_condition = len(c_indexer) + 1
        else:
            n_condition = len(c_indexer)
            assert c_indexer['<nan>'] == 0  # Check
        ...
        with open(os.path.join(args.data_dir, args.input_file), encoding="utf-8") as fin:
            # *ZY*
            input_lines = [line.strip().split('\t')[:2] for line in fin.readlines()]
            if args.subset > 0:
        ...
        input_lines = [[data_tokenizer.tokenize(
            src)[:max_src_length], uid] for src, uid in input_lines]
        ...
        with tqdm(total=total_batch) as pbar:
            while next_i < len(input_lines):
                ...
                instances = []
                for instance in [(x[0], x[1], max_a_len) for x in buf]:
                ...
                with torch.no_grad():
                    ...
                    input_ids, usrid_ids, token_type_ids, position_ids, input_mask, mask_qkv, task_idx = batch
                    traces = model(input_ids, usrid_ids, token_type_ids, position_ids, input_mask,
                                   task_idx=task_idx, mask_qkv=mask_qkv)

                    if args.beam_size > 1:
                        traces = {k: v.tolist() for k, v in traces.items()}
                        output_ids = traces['pred_seq']
                        # print(output_ids)  # Debug
                    else:
        ...
        with open(fn_out, "w", encoding="utf-8") as fout:
            for l in output_lines:
                ...
                len_list.append(len(l.strip().split(' ')))

        print("### average len: {:}".format(np.mean(len_list)))
