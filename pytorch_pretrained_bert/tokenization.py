...
class BertTokenizer(object):
    ...
    def __init__(self, vocab_file, do_lower_case=True, max_len=None, never_split=("[UNK]", "[SEP]", "[X_SEP]", "[PAD]", "[CLS]", "[MASK]")):
        ...
        # *ZY*
        self.my_special = list(never_split) + ['<url>', '<person>', '<num>', '<nan>', '[SRC_SEP]']
        self.vocab['<url>'] = 20
        self.vocab['<person>'] = 21
        self.vocab['<num>'] = 22
        self.vocab['<nan>'] = 23
        self.vocab['[SRC_SEP]'] = 24

        del self.vocab['[unused19]']
        del self.vocab['[unused20]']
        del self.vocab['[unused21]']
        del self.vocab['[unused22]']
        del self.vocab['[unused23]']

        self.ids_to_tokens = collections.OrderedDict(
            [(ids, tok) for tok, ids in self.vocab.items()])
        self.basic_tokenizer = BasicTokenizer(
            do_lower_case=do_lower_case, never_split=self.my_special)  # *ZY*
    ...
    def convert_tokens_to_ids_FGfree(self, tokens, ret_ids_only=True):
        """Converts a sequence of tokens into ids using the vocab."""
        ids = []
        position_ids = []
        mask_pos_idx_map = {}
        ...
        for pos, token in enumerate(tokens):
            if isinstance(token, str):
                ...
                idx_counter += 1
            else:  # TODO: masked tokens -- here is tuple
                mask_pos_idx_map[pos] = idx_counter
                assert len(token) == 2
                for t in token:  # here is tuple, and the first position is [MASK]
        ...
        return ids, position_ids, mask_pos_idx_map
