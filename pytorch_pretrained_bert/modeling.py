...
# *ZY*: It's difficult to install apex on Compute Canada.
# try:
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, usrid_ids=None,
        ...
        # attention_head_size: the dimension of query and key
        ...
        # (batch, pos, all_head_size)
        context_layer = context_layer.view(*new_context_layer_shape)

        return context_layer


class BertSelfAttentionConditioned(BertSelfAttention):
    def __init__(self, config):
        super(BertSelfAttentionConditioned, self).__init__(config)
        # Condition Embed
        self.c_value = nn.Embedding(config.n_condition, config.hidden_size)
        self.c_key = nn.Embedding(config.n_condition, config.hidden_size)

        try:
            self.gate = config.gate
        except AttributeError:
            print("gate AttributeError")
            self.gate = 'attn'

        print("### Gate Method: {:}".format(self.gate))

        if self.gate == 'attn':
            pass

        elif self.gate == 'gate':
            self.get_beta = nn.Linear(config.hidden_size, 1)

        elif self.gate == 'gate_x2':
            self.get_alpha = nn.Linear(config.hidden_size, 1)
            self.get_beta = nn.Linear(config.hidden_size, 1)

        else:
            raise ValueError

    def forward(self, hidden_states, attention_mask, cids=None,
                history_states=None, mask_qkv=None, seg_ids=None):
        ...
        cids = cids.view(batch_size, 1)

        if self.gate == 'attn':
            cids = F.pad(cids, (1, 0))  # (batch, 2)

            c_keys = self.c_key(cids)  # (batch, 2, all_head_size)
            c_attention_scores = torch.matmul(
                context_layer / math.sqrt(self.all_head_size), c_keys.transpose(-1, -2))
            # (batch, pos, 2)

            c_attention_probs = nn.Softmax(dim=-1)(c_attention_scores)

            c_vals = self.c_value(cids)
            mask = cids.view(batch_size, 2, 1) != 0
            mask = mask.type_as(c_vals).detach()
            c_vals = c_vals * mask

            c_context_bias = torch.matmul(c_attention_probs, c_vals)

        elif self.gate == 'gate':
            beta = torch.sigmoid(self.get_beta(context_layer))  # (batch, pos, 1)
            c_context_bias = beta * self.c_value(cids)  # (batch, 1, d)

        elif self.gate == 'gate_x2':
            c_context_bias = self.c_value(cids)  # (batch, 1, d)
            alpha = torch.sigmoid(self.get_alpha(context_layer))  # (batch, pos, 1)
            beta = torch.sigmoid(self.get_beta(c_context_bias))  # (batch, 1, 1)

        else:
            raise ValueError

        # assert c_context_bias.shape == context_layer.shape
        # # (batch, pos, all_head_size)

        # Mask where seg_ids (batch, pos) is 0
        mask = seg_ids.unsqueeze(dim=2) != 0
        mask = mask.type_as(c_context_bias).detach()
        c_context_bias = c_context_bias * mask

        if self.gate == 'gate_2x':
            return alpha * context_layer + beta * c_context_bias
        else:
            return context_layer + c_context_bias
...
class BertAttention(nn.Module):
    def __init__(self, config, is_conditioned=False):
        super(BertAttention, self).__init__()
        self.is_conditioned = is_conditioned
        if self.is_conditioned:
            self.self = BertSelfAttentionConditioned(config)
        else:
            self.self = BertSelfAttention(config)

        self.output = BertSelfOutput(config)

    def forward(self, input_tensor, attention_mask, usrid_ids,
                history_states=None, mask_qkv=None, seg_ids=None):

        self_output = self.self(
            input_tensor, attention_mask, usrid_ids,
            history_states=history_states, mask_qkv=mask_qkv, seg_ids=seg_ids)
...
class BertLayer(nn.Module):
    def __init__(self, config, is_conditioned=False):
        ...
        self.attention = BertAttention(config, is_conditioned=is_conditioned)
        self.ffn_type = config.ffn_type
        if self.ffn_type:
    ...
    def forward(self, hidden_states, attention_mask, usrid_ids, history_states=None, mask_qkv=None, seg_ids=None):
        attention_output = self.attention(
            hidden_states, attention_mask, usrid_ids,
            history_states=history_states, mask_qkv=mask_qkv, seg_ids=seg_ids)
        if self.ffn_type:
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        c_layer = BertLayer(config, is_conditioned=True)

        clayer_list = [copy.deepcopy(c_layer) for _ in range(config.n_clayer-1)]
        for clayer in clayer_list:
            clayer.attention.self.c_value = c_layer.attention.self.c_value
            clayer.attention.self.c_key = c_layer.attention.self.c_key

            try:
                clayer.attention.self.get_alpha = c_layer.attention.self.get_alpha
            except AttributeError:
                pass

            try:
                clayer.attention.self.get_beta = c_layer.attention.self.get_beta
            except AttributeError:
                pass

        self.layer = nn.ModuleList([copy.deepcopy(layer)
                                    for _ in range(config.num_hidden_layers-config.n_clayer)] +
                                   [c_layer] + clayer_list)

    def forward(self, hidden_states, attention_mask, usrid_ids, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, seg_ids=None):
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for layer_module in self.layer:
                hidden_states = layer_module(
                    hidden_states, attention_mask, usrid_ids,
                    mask_qkv=mask_qkv, seg_ids=seg_ids)
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights, num_labels=2):
        ...
        self.predictions = BertLMPredictionHead(
            config, bert_model_embedding_weights)
        self.seq_relationship = nn.Linear(config.hidden_size, num_labels)  # Next Sentence Prediction
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        # TODO: *ZY*
        config.n_condition = kwargs['n_condition']
        config.n_dial = kwargs['n_dial']
        config.n_clayer = kwargs['n_clayer']
        config.gate = kwargs['gate']
        ...
        for arg_clean in ('gate', 'n_clayer', 'n_dial', 'n_condition', 'config_path', 'type_vocab_size', 'relax_projection', 'new_pos_ids', 'task_idx', 'max_position_embeddings', 'fp32_embedding', 'ffn_type', 'label_smoothing', 'hidden_dropout_prob', 'attention_probs_dropout_prob', 'num_qkv', 'seg_emb', 'word_emb_map'):
        ...
        for key in state_dict.keys():
            new_key = None
            if 'gamma' in key:
                new_key = key.replace('gamma', 'weight')  # unilm add these codes not me
            if ('beta' in key) and ('attention.self.get_beta' not in key):  # i add these
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, usrid_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, mask_qkv=None, task_idx=None):
...
class BertModelIncr(BertModel):
    ...
    def forward(self, input_ids, usrid_ids, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=True, prev_embedding=None,
        ...
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask, usrid_ids,
...
class BertForPreTrainingLossMask(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        ...
        self.cls2 = None
        if self.num_sentlvl_labels > 0:  # *ZY*: False
    ...
    def forward(self, input_ids, usrid_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                ...
                num_tokens_a=None, num_tokens_b=None, mask_qkv=None, is_ppl_eval=False):

        # print("Model task id {:}".format(task_idx))

        if attention_mask is None:  # TODO: Pretraining
            task_0 = (task_idx == 0)
            task_1 = (task_idx == 1)
            task_2 = (task_idx == 2)
            ...
            tril = index_matrix <= index_matrix_t  # i.e. [[[1,0], [1,1]]]
        ...
        sequence_output, pooled_output = self.bert(
            input_ids, usrid_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx)
        ...
        def loss_mask_and_normalize(loss, mask, ret_dial_loss=False):
            ...
            # *ZY*
            if ret_dial_loss:
                dial_loss = node[:self.n_dial, :]
                # print(self.n_dial)
                # print("Shape {:} {:}".format(node.shape, dial_loss.shape))
                return node.sum(), dial_loss.sum()
            else:
                return node.sum()
        ...
        if is_ppl_eval:
            loss_ppl = masked_lm_loss.float()
            mask_ppl = masked_weights.type_as(loss_ppl)
            loss_ppl = loss_ppl * mask_ppl
            loss_ppl = loss_ppl.sum()

        masked_lm_loss, dial_loss_sum = loss_mask_and_normalize(
            masked_lm_loss.float(), masked_weights, ret_dial_loss=True)
        ...
        if is_ppl_eval:
            return masked_lm_loss, next_sentence_loss, loss_ppl  # *ZY*

        if pair_x is None or pair_y is None or pair_r is None or pair_pos_neg_mask is None or pair_loss_mask is None:
            return masked_lm_loss, next_sentence_loss, dial_loss_sum  # *ZY*
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def forward(self, input_ids, usrid_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        if self.search_beam_size > 1:
            return self.beam_search(input_ids, usrid_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx, mask_qkv=mask_qkv)
        ...
        while next_pos < output_length:
            ...
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, usrid_ids, curr_token_type_ids, curr_position_ids, curr_attention_mask,
    ...
    def beam_search(self, input_ids, usrid_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        input_shape = list(input_ids.size())
        batch_size = input_shape[0]
        usrid_ids = usrid_ids.view(batch_size, -1)  # *ZY*
        input_length = input_shape[1]
        output_shape = list(token_type_ids.size())
        output_length = output_shape[1]
        ...
        while next_pos < output_length:
            ...
            # *ZY*
            n_id_each = 1
            if x_input_ids.shape[0] != usrid_ids.shape[0]:
                # beam search
                # cids = cids.expand(cids.shape[0], K).contiguous().view(-1, 1)
                usrid_ids = usrid_ids.unsqueeze(dim=1)
                usrid_ids = torch.flatten(usrid_ids.expand(batch_size, K, n_id_each).contiguous(), end_dim=1)

                assert x_input_ids.shape[0] == usrid_ids.shape[0]
